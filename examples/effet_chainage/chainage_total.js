...
//liste des effets à appliquer sur la vidéo 1 puis sur la vidéo 2
let L1 = ['NoirBlanc', 'Negatif'];
let L2 = ['CodeFlou'];

//transition à appliquer
let transition = 'Transition_fondu';
...
for (i=0; i<L1.length; i++) {
	counter++;
	import('../../Codes_separes/' + L1[i] + '.js')
       .then( obj => { print('Module loaded: ' + obj.name + ' nb uniforms ' + obj.uniforms.length); effet.push(obj); counter--;})
       .catch( err => { counter--; print('Failed to load object ' + obj.name + err); });
}
for (i=0; i<L2.length; i++) {
	counter++;
	import('../../Codes_separes/' + L2[i] + '.js')
       .then( obj => { print('Module loaded: ' + obj.name + ' nb uniforms ' + obj.uniforms.length); effet.push(obj); counter--;})
       .catch( err => { counter--; print('Failed to load object ' + obj.name + err); });
}
counter++;
import('../../Codes_separes/' + transition + '.js')
       .then( obj => { print('Module loaded: ' + obj.name + ' nb uniforms ' + obj.uniforms.length); effet.push(obj); counter--;})
       .catch( err => { counter--; print('Failed to load object ' + obj.name + err); });


//besoin de buffer
// 0 pour aucun buffer, 1 pour buffer pour la vidéo 1, 2 pour buffer pour la vidéo 2
// on suppose pour l'instant qu'on aura besoin d'un buffer pour une seule des 2 vidéos
// on suppose que si un effet a besoin d'un buffer, il est applqué en dernier

let buffer = 0;

if (L1.length>1) {
	if(L1[L1.length-1].buffer)
		buffer = 1;
}

else if (L2.length>1) {
	if(L2[L2.length-1].buffer)
		buffer = 2;
}

function make_fs_source_0() {
	let i = 0;
	let uniforms = ``;
	let code1 = ``;
	let code2 = ``;
	for (i=0; i<effet.length; i++) {
		uniforms += effet[i].uniforms.name;
	}
	for (i=0; i<L1.length; i++) {
		code1 += effet[i].code;
	}
	for (i=L1.length; i<effet.length-1; i++) {
		code2 += effet[i].code;
	}
	let fsSource = `varying vec2 vTextureCoord;
 uniform sampler2D vidTx1;
 uniform sampler2D vidTx2;
` + uniforms + `void main(void) {
	vec2 tx = vTextureCoord;
` + effet[effet.length-1].codetx + `
	// effets vidéo 1
	sampler2D inTexture = vidTx1;
	vec4 vid = texture2D(vidTx1, inTextureCoord);
` + code1 + `
	vec4 result1 = vid;
...
'vec4 result2 = vid;
	...
	// transition
` + effet[effet.length-1].code + `gl_FragColor = result;
}`;

	return fsSource;
}

function make_fs_source_1() {
	let i = 0;
	let uniforms = ``;
	let code1 = ``;
	let code2 = ``;
	...
	let fsSource1 = `varying vec2 vTextureCoord;
 uniform sampler2D vidTx1;
` + uniforms + `void main(void) {
	vec2 tx = vTextureCoord;
` + effet[effet.length-1].codetx + `
...
` + code1 + `
	...
	let fsSource2 = `varying vec2 vTextureCoord;
 uniform sampler2D vidTx2;
...
` + effet[effet.length-1].codetx + `
	// effets vidéo 1
	sampler2D inTexture = imgTx;
	vec4 vid = texture2D(imgTx, inTextureCoord);
` + effet[L1.length-1].code + `
	vec4 result1 = vid;
...
` + effet[effet.length-1].code + `gl_FragColor = result;
	...
	return [fsSource1, fsSource2];
}

function make_fs_source_2() {
	let i = 0;
	let uniforms = ``;
	let code1 = ``;
	let code2 = ``;
	...
	for (L1.length; i<effet.length-2; i++) {
		code2 += effet[i].code;
	}
...
` + effet[effet.length-1].codetx + `
	// effets vidéo 2
	sampler2D inTexture = vidTx2;
	vec4 vid = texture2D(vidTx2, inTextureCoord);
` + code2 + `
	gl_FragColor = vid;`;	
...
filter.process = function()
{
  ...
  for (let i=0; i<pids.length; i++) {
    ...
    if (!program_pass1) {
		...
		if(buffer==0) {
			program_pass1 = setupProgram(gl, vsSource, make_fs_source_0());
			...
			program_pass1 = setupProgram(gl, vsSource, make_fs_source_1()[0]);
      		program_pass1.uniformLocations.texture1 = gl.getUniformLocation(program_pass1.program, 'vidTx1');

      		program_pass2 = setupProgram(gl, vsSource, make_fs_source_1()[1]);
      		program_pass2.uniformLocations.texture1 = gl.getUniformLocation(program_pass2.program, 'imgTx');
      		program_pass2.uniformLocations.texture2 = gl.getUniformLocation(program_pass2.program, 'vidTx2');
		}
		
		else {
			program_pass1 = setupProgram(gl, vsSource, make_fs_source_2()[0]);
      		program_pass1.uniformLocations.texture1 = gl.getUniformLocation(program_pass1.program, 'vidTx2');

      		program_pass2 = setupProgram(gl, vsSource, make_fs_source_2()[0]);
...
function drawScene(gl, prog_info, tx1, tx2)
{
  ...
  let j = 0;
  for (i=0; i<effet.length; i++) {
	for (j=0; j<effet[i].uniforms.length; j++) {
		if (effet[i].uniforms.type == '1f') {
			//gl.uniform1f(prog_info.uniformLocations.(   ), effet[i].uniforms.default);
		}
		else if (effet[i].uniforms.type == '1fv') {
			//gl.uniform1fv(prog_info.uniformLocations.(   ), effet[i].uniforms.default);
		}
		else {
			//gl.uniform2fv(prog_info.uniformLocations.(   ), effet[i].uniforms.default);
		}
	}
}
