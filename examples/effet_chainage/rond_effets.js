...
function drawScene(gl, programInfo, buffers)
{
  ...
  gl.uniform1f(programInfo.uniformLocations.largeur, height);
...
void main(void) {
  ...
  if ((gl_FragCoord.x-width/2.0)*(gl_FragCoord.x-width/2.0)+(gl_FragCoord.y-height/2.0)*(gl_FragCoord.y-height/2.0) < 800.0*800.0*seuil*seuil) {
	vid = texture2D(vidTx1, tx);
  vid.rgb=
  }
  else {
	vid = texture2D(vidTx2,tx);
  }
  gl_FragColor = effet_BAndW(vid, vTextureCoord);
}
`;
